...
function PaymentSuccessContent() {
  ...
  const [paymentData, setPaymentData] = useState<TransactionType | null>(null);
  ...
  useEffect(() => {
    ...
    const getPaymentDataBySessionId = async () => {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/payment/transaction/${sessionId}`
      );
      const data =
        (await response.json()) as TransactionResponse<TransactionType>;
      setPaymentData(data.transaction);
      setIsLoading(false);
    };

    getPaymentDataBySessionId();
  }, [searchParams, router]);
            ...
            >
              <div className="text-center mb-8">
                <div
                  className={`inline-flex items-center justify-center w-20 h-20 bg-gradient-to-r ${
                    tierColorMapping[paymentData.subscription.tier]
                  } rounded-full mb-4`}
                >
                  {tierIconMapping[paymentData.subscription.tier]}
                </div>
                <h2 className="text-3xl font-bold text-white mb-2">
                  {paymentData.subscription.tier
                    .toLowerCase()
                    .charAt(0)
                    .toUpperCase() +
                    paymentData.subscription.tier.slice(1)}{" "}
                  Membership
                </h2>
              ...
              <div className="grid md:grid-cols-2 gap-8">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    {tierBenefits[
                      paymentData.subscription.tier.toLowerCase()
                    ]?.map((benefit, index) => (
      ...
      {paymentData.userPackage && (
        <div className="container mx-auto px-4 pb-12">
          ...
          >
            <div className="text-center mb-8">
              <div
                className={`inline-flex items-center justify-center w-20 h-20 bg-gradient-to-r rounded-full mb-4`}
              >
                <Star className="w-8 h-8 text-amber-500" />
              </div>
              <h2 className="text-3xl font-bold text-white mb-2">
                {paymentData.userPackage.name
                  .toLowerCase()
                  .charAt(0)
                  .toUpperCase() + paymentData.userPackage.name.slice(1)}{" "}
                Membership
              </h2>
              <p className="text-gray-300">Your Package is now active</p>
            </div>
